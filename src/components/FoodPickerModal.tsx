...
export function FoodPickerModal({ open, foods, query, onQueryChange, onClose, onSelect }: Props) {
  return (
    <AppModal open={open} fullscreen title="Aggiungi alimento" eyebrow="COMPOSIZIONE MANUALE" onClose={onClose}>
      <div className="search-modal-sticky">
        <div className="search-shell modal-search">
          <span className="search-icon">⌕</span>
          <input placeholder="Cerca alimento o barcode..." value={query} onChange={(e) => onQueryChange(e.target.value)} autoFocus inputMode="search" />
        </div>
      </div>
      <div className="picker-list modal-picker-list">
        {foods.map((food) => <button key={food.id} onClick={() => onSelect(food)}>
