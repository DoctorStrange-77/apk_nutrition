...
const StepState = ({ done }: { done: boolean }) => (
  <span className={`setup-step-state ${done ? 'done' : ''}`}>{done ? 'âœ“' : 'â€¢'}</span>
);
...
export function SetupGuideCard(props: Props) {
  ...
  return <section className="setup-guide-card">
    <div className="setup-guide-head">
      <div><p className="eyebrow red">PRIMO UTILIZZO</p><h2>Configura Builder in 3 passaggi</h2></div>
      <span className="setup-progress-badge">{Number(targetReady) + Number(timingReady) + Number(generated)}/3</span>
    </div>
    <p className="setup-guide-intro">Per creare automaticamente il menu servono prima i tuoi macro giornalieri e il timing dei pasti.</p>
    <div className={`setup-step ${targetReady ? 'done' : 'current'}`}>
      <StepState done={targetReady} />
      <div className="setup-step-copy"><strong>1. Imposta i macro di partenza</strong><span>{targetReady ? 'Target confermato.' : 'Calcolali dal profilo oppure inseriscili manualmente.'}</span></div>
    ...
    <div className={`setup-step ${timingReady ? 'done' : targetReady ? 'current' : 'locked'}`}>
      ...
      <div className="setup-step-copy"><strong>2. Scegli il Timing</strong><span>{timingReady ? activeTimingName : 'Seleziona quando ti alleni e come distribuire i macro nei pasti.'}</span></div>
    ...
    <div className={`setup-step ${generated ? 'done' : readyToGenerate ? 'current' : 'locked'}`}>
      ...
      <div className="setup-step-copy"><strong>3. Genera il primo menu</strong><span>{generated ? 'Configurazione completata.' : 'Builder userÃ  macro + timing per creare automaticamente la giornata.'}</span></div>
